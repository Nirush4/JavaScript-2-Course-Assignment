...
const safe = (v?: string | null, fallback = '') => (v && String(v).trim() ? String(v) : fallback);
...
function media(post: Post): { url: string; alt: string } {
	...
	if (m && typeof m === 'object') return { url: m.url || '', alt: m.alt || post.title || 'media' };
...
export default function postCard(p: Post, _index?: number) {
  ...
  <article
    ...
    <header class="flex items-center gap-3 mb-3">
      ...
      <div>
        <div class="font-semibold clamp-1">${safe(p.author?.name, 'Unknown')}</div>
        <div class="text-xs opacity-70">${new Date(p.created).toLocaleString()}</div>
      </div>
    </header>
    ...
    ${p.title ? `<h3 class="text-lg font-bold mb-1 clamp-1">${p.title}</h3>` : ''}
    ${p.body ? `<p class="opacity-90 mb-3 whitespace-pre-wrap clamp-3">${p.body}</p>` : ''}
    ...
    <div class="mt-auto">
      ...
      <form data-comment-form data-post-id="${p.id}" class="flex items-center gap-2">
      ...
      <ul data-comment-list class="space-y-3 hidden mt-3">
        ${(p.comments ?? [])
					...
					.map(c => {
						...
						return `
            <li class="comment rounded-lg bg-white/5 p-3" data-id="${c.id}" data-owner="${c.owner}">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  <div class="text-sm">
                    ...
                    <span class="opacity-70">• ${new Date(c.created).toLocaleString()}</span>
                  </div>
                  <div class="comment-body whitespace-pre-wrap">${c.body ?? ''}</div>
                  <div class="mt-2">
                    ${
											canDelete
												? `
