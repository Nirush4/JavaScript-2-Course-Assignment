import { toggleReaction, addComment, deleteComment } from '../services/posts/interactions';
...
function showUndoToast(msg: string, onUndo: () => void | Promise<void>, ms = 5000) {
	const bar = document.createElement('div');
	bar.className =
		'fixed bottom-4 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-lg bg-black/80 text-white shadow-lg flex items-center gap-4';
	bar.innerHTML = `<span>${msg}</span>
    <button type="button" data-undo class="px-3 py-1 rounded bg-white/10 hover:bg-white/20">${TEXT.undo}</button>`;
...
export function bootFeedInteractions() {
	...
	document.addEventListener('click', e => {
		const btn = (e.target as HTMLElement).closest<HTMLElement>('[data-comments-toggle]');
		if (!btn) return;
	...
	document.addEventListener('click', async e => {
		const btn = (e.target as HTMLElement).closest<HTMLButtonElement>('[data-like-btn]');
		if (!btn) return;
		...
		const wasLiked =
			btn.classList.contains('is-liked') || btn.getAttribute('aria-pressed') === 'true' || btn.dataset.liked === '1';
	...
	document.addEventListener('submit', async e => {
		const form = (e.target as HTMLElement).closest<HTMLFormElement>('[data-comment-form]');
		if (!form) return;
		...
		const input = form.querySelector<HTMLInputElement | HTMLTextAreaElement>('[name="comment"]');
		if (!input) return;
		...
		if (list.classList.contains('hidden')) {
			...
			const toggle = card?.querySelector<HTMLElement>('[data-comments-toggle] .underline');
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="comment-body whitespace-pre-wrap">${escapeHtml(body)}</div>
        </div>
      </div>
    `;
		list.prepend(tempLi);
		...
		try {
			...
			tempLi.innerHTML = `
        <div class="flex items-start gap-3">
          <img src="${created.author?.avatar || AVATAR_32}" class="size-8 rounded-full" alt="" />
          <div class="flex-1">
            <div class="text-sm">
              <span class="font-semibold">@${escapeHtml(created.owner ?? displayName)}</span>
              <span class="opacity-70">• ${
								created.created ? new Date(created.created).toLocaleString() : TEXT.justNow
							}</span>
            </div>
            <div class="comment-body whitespace-pre-wrap">${escapeHtml(created.body)}</div>
            <div class="mt-2">
              <button type="button" class="text-xs opacity-70 hover:opacity-100"
                      data-delete-comment data-post-id="${postId}" data-comment-id="${created.id}">
                Delete
              </button>
            </div>
          </div>
	...
	document.addEventListener('click', async e => {
		const btn = (e.target as HTMLElement).closest<HTMLButtonElement>('[data-delete-comment]');
		if (!btn) return;
		...
		const snapshotBody = li.querySelector<HTMLElement>('.comment-body')?.textContent ?? '';
		...
		try {
			...
			showUndoToast(TEXT.delComment, async () => {
				...
				const list = document.querySelector<HTMLElement>(`[data-post-id="${postId}"] [data-comment-list]`);
				if (!list) return;
				...
				restored.innerHTML = `
          <div class="flex items-start gap-3">
            <img src="${recreated.author?.avatar || AVATAR_32}" class="size-8 rounded-full" alt="" />
            <div class="flex-1">
              <div class="text-sm">
                <span class="font-semibold">@${escapeHtml(recreated.owner ?? me ?? TEXT.you)}</span>
                <span class="opacity-70">• ${
									recreated.created ? new Date(recreated.created).toLocaleString() : TEXT.justNow
								}</span>
              </div>
              <div class="comment-body whitespace-pre-wrap">${escapeHtml(recreated.body)}</div>
              <div class="mt-2">
                <button type="button" class="text-xs opacity-70 hover:opacity-100"
                        data-delete-comment data-post-id="${postId}" data-comment-id="${recreated.id}">
                  Delete
                </button>
              </div>
            </div>
