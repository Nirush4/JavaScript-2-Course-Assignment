...
type PostItem = {
	...
	body?: string;
	tags?: string[];
	media?: { url?: string; alt?: string } | Array<{ url?: string; alt?: string }>;
	author?: { name?: string };
	created?: string;
	updated?: string;
};
...
export default async function ProfilePage(): Promise<string> {
	...
	try {
		profile = await api<ProfileResponse>(`/profiles/${encodeURIComponent(username)}`);
	} catch (err: any) {
		return `
      <main class="min-h-dvh bg-gray-900 text-gray-100 p-8">
        <h1 class="text-3xl font-bold mb-6">Your Profile</h1>
        <div class="p-4 rounded bg-red-900/30 border border-red-700">
          ...
          <code class="text-sm break-all">${escHtml(err?.message ?? String(err))}</code>
        </div>
	...
	let posts: PostItem[] = [];
	try {
		const res = await api<any>(`/profiles/${encodeURIComponent(username)}/posts?limit=50&sort=created&sortOrder=desc`);
	...
	const avatar =
		(typeof profile?.avatar === 'string' ? profile.avatar : profile?.avatar?.url) || '/public/profile.avif';
    ...
    <main class="min-h-dvh bg-gray-900 text-gray-100 px-4 sm:px-6 py-6">
      ...
      <header class="max-w-3xl mx-auto rounded-2xl border border-gray-800 bg-gray-850/60 p-6 shadow mb-8">
        <div class="flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h1 class="text-3xl sm:text-4xl font-extrabold tracking-tight">${escHtml(displayName)}</h1>
            <p class="opacity-80 text-sm sm:text-base mt-2">${bio ? escHtml(bio) : '—'}</p>
        ...
        <div id="posts" class="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
          ${
						posts.length
							? posts
									.map(p => {
										const media = Array.isArray(p.media) ? p.media?.[0]?.url : (p.media as any)?.url;
                    ...
                    <article class="rounded-xl bg-gray-850/60 border border-gray-800 p-4 hover:border-gray-700 transition shadow">
                      ...
                      <div class="text-xs opacity-60 mb-2">${escHtml(created)}</div>
                      <div class="text-sm opacity-90 line-clamp-3">${escHtml(p.body ?? '')}</div>
                      ${tags}
                      ${
												own
...
function wireProfileHandlersOnce() {
	...
	document.addEventListener('click', async e => {
		...
		if (editBtn) {
			...
			if (!id) return;
			try {
				const res = await api<any>(`/posts/${id}?_author=true&_reactions=true&_comments=true`);
	...
	document.addEventListener('submit', async e => {
		...
		payload.media = imgUrl ? { url: imgUrl, ...(imgAlt ? { alt: imgAlt } : {}) } : null;
