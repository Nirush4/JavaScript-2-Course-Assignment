...
export default async function ProfilePage(): Promise<string> {
  ...
  if (!username) {
    return `
      <main role="main" aria-label="Profile page" class="min-h-dvh bg-gray-900 text-gray-100 p-8">
        ...
        <a class="inline-block mt-6 underline focus:outline focus:ring-2 focus:ring-blue-400" href="/login">→ Go to Login</a>
      </main>
    `;
  }
  ...
  try {
    ...
    return `
      <main role="main" aria-label="Profile page" class="min-h-dvh bg-gray-900 text-gray-100 p-8">
        <h1 class="text-3xl font-bold mb-6">Your Profile</h1>
        <div class="p-4 rounded bg-red-900/30 border border-red-700" role="alert" aria-live="assertive">
  ...
  const html = `
    <main role="main" aria-label="User Profile Page" class="relative min-h-dvh bg-gradient-to-b from-gray-950 via-gray-900 to-gray-800 text-gray-100">
      ...
      <header class="sticky top-0 z-50 w-full bg-gray-900/70 backdrop-blur-md border-b border-gray-800 shadow-sm">
        <div class="max-w-6xl mx-auto flex items-center justify-between px-6 py-4">
          <nav class="flex items-center gap-6" aria-label="Main navigation">
            <a href="/feed" class="text-gray-300 hover:text-blue-400 md:text-lg font-medium focus:outline focus:ring-2 focus:ring-blue-400">🏠 Feed</a>
            <a href="/profile" class="text-gray-300 hover:text-blue-400 md:text-lg font-medium focus:outline focus:ring-2 focus:ring-blue-400">👤 Profile</a>
          </nav>
          <h1 class="text-lg sm:text-xl font-semibold text-blue-300 tracking-tight">${escHtml(
            displayName
          )}</h1>
          <button id="logout-btn" class="px-4 py-1.5 rounded-md bg-red-600/90 hover:bg-red-500 text-white font-medium text-sm transition-colors duration-200 focus:outline focus:ring-2 focus:ring-red-400 md:text-lg">
            Logout
          </button>
        </div>
      </header>
      ...
      <section class="px-4 sm:px-6 py-10">
        <header class="max-w-3xl mx-auto mt-6 rounded-2xl border border-gray-800 bg-gray-900/60 backdrop-blur-md p-8 sm:p-10 shadow-xl hover:shadow-2xl transition-all duration-300" role="region" aria-label="Profile Info">
    ...
    typeof profile?.avatar === 'string'
      ...
      : profile?.avatar?.alt ?? 'Avatar'
            ...
            <div>
              <h2 class="text-3xl sm:text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-teal-300">${escHtml(
                displayName
              )}</h2>
              <p class="opacity-80 text-sm sm:text-base mt-3 max-w-md mx-auto leading-relaxed">${escHtml(
                bio
              )}</p>
              ...
              <div class="mt-6">
                <button id="toggle-create" aria-pressed="false" class="px-6 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium shadow hover:from-blue-500 hover:to-indigo-500 focus:outline focus:ring-2 focus:ring-blue-400 transition-all duration-200">
                  + New Post
                </button>
              </div>
            </div>
        ...
        <section id="editor" class="max-w-2xl mx-auto mt-10 hidden" role="region" aria-label="Post Editor">
          <div class="rounded-2xl border border-gray-800 bg-gray-900/70 backdrop-blur-md p-6 sm:p-8 shadow-2xl">
            ...
            <form id="post-form" class="space-y-4" aria-live="polite">
              <input type="hidden" id="postId" value="">
              <div>
                <label class="block mb-1 text-sm font-medium text-gray-300" for="title">Title <span class="text-red-400">*</span></label>
                <input id="title" required aria-required="true" aria-invalid="false"
                       class="w-full p-2.5 rounded-lg border border-gray-700 bg-gray-800 placeholder-gray-500 focus:outline focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
              </div>
              <div>
                <label class="block mb-1 text-sm font-medium text-gray-300" for="body">Body</label>
                <textarea id="body" rows="6"
                          class="w-full p-2.5 rounded-lg border border-gray-700 bg-gray-800 placeholder-gray-500 focus:outline focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"></textarea>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label class="block mb-1 text-sm font-medium text-gray-300" for="imgUrl">Image URL</label>
                  <input id="imgUrl" placeholder="https://..."
                         class="w-full p-2.5 rounded-lg border border-gray-700 bg-gray-800 focus:outline focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                </div>
                <div>
                  <label class="block mb-1 text-sm font-medium text-gray-300" for="imgAlt">Image ALT</label>
                  <input id="imgAlt"
                         class="w-full p-2.5 rounded-lg border border-gray-700 bg-gray-800 focus:outline focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                </div>
              </div>
              <div>
                <label class="block mb-1 text-sm font-medium text-gray-300" for="tags">Tags (comma separated)</label>
                <input id="tags"
                       class="w-full p-2.5 rounded-lg border border-gray-700 bg-gray-800 focus:outline focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
              ...
              <div class="flex items-center gap-3 pt-2">
                <button type="submit" id="save-btn"
                        class="px-5 py-2 rounded-lg bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium hover:from-green-500 hover:to-emerald-500 transition-all focus:outline focus:ring-2 focus:ring-green-400">
                  Create
                </button>
                <button type="button" id="cancel-edit"
                        class="px-4 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 transition-all focus:outline focus:ring-2 focus:ring-gray-500">
                  Cancel
                </button>
                <button type="button" id="reset-edit"
                        class="px-4 py-2 rounded-lg border border-gray-600 bg-gray-800 hover:bg-gray-700 transition-all focus:outline focus:ring-2 focus:ring-gray-500">
                  Reset
                </button>
                <span id="msg" class="text-sm opacity-80 ml-2 text-yellow-400" role="status" aria-live="polite"></span>
              </div>
            </form>
          </div>
        </section>

        <section class="max-w-6xl mx-auto mt-12" aria-label="User Posts">
                ...
                ? posts
                    .map((p) => {
                      const media = Array.isArray(p.media)
                        ? p.media?.[0]?.url
                        : (p.media as any)?.url;
                      const altText = Array.isArray(p.media)
                        ? p.media?.[0]?.alt ?? 'Post image'
                        : (p.media as any)?.alt ?? 'Post image';
                      ...
                      const tags =
                        Array.isArray(p.tags) && p.tags.length
                          ...
                          : '';
                      return `
                      <article class="rounded-2xl bg-gray-900/60 border border-gray-800 p-5 shadow-lg hover:shadow-blue-900/30 hover:border-blue-600/50 transition-all duration-300" role="region" aria-label="Post: ${escHtml(
                        p.title
                      )}">
                        ${
                          media
                        ...
                        ${
                          ...
                          <button class="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 transition-all js-edit cursor-pointer focus:outline focus:ring-2 focus:ring-blue-400" data-id="${p.id}">Edit</button>
                          <button class="px-3 py-1.5 rounded-lg bg-red-600 hover:bg-red-500 transition-all js-delete cursor-pointer focus:outline focus:ring-2 focus:ring-red-400" data-id="${p.id}">Delete</button>
                        </div>`
                            : ''
                        }
                      </article>
      ...
      <div class="max-w-6xl mx-auto text-center pb-10">
        <a href="/feed" class="inline-flex items-center gap-2 text-lg font-medium text-blue-400 hover:text-blue-300 focus:outline focus:ring-2 focus:ring-blue-400 transition-colors duration-200">
          ← Back to Feed
        </a>
      </div>
    </main>
